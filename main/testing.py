...
def extract_faces_from_video(video_path, output_dir, frame_skip=16):
...
if __name__ == "__main__":
    ...
    #test_video_file = "./videos/01_04__walking_and_outside_surprised__GBC7ZGDP.mp4" 

        if not os.path.exists(vid):
            print(f"Error: Video file not found at {vid}. Please update the path.")
            exit()
        ...
        # Make prediction
        result = predict_video(vid, model, device=device)
    ...
    precision, recall, f_score, _ = precision_recall_fscore_support(values, preds)
